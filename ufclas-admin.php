...
Description: Management Tasks for UF CLAS.
Version: 0.0.1
...
function ufclas_admin_register_menu(){
	add_menu_page('CLAS Admin Tools', 'CLAS Admin', 'manage_options', 'ufclas-admin-tools', 'ufclas_admin_tools_page');
}

function ufclas_admin_tools_page(){
	if ( !current_user_can( 'manage_options' ) )  {
		wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
	}
	$default_usernames = array('clasnews','mailchimp','cismac','slideshow');
	
	?>
	<div class="wrap">
		<div id="icon-tools" class="icon32"></div>
		<h2><?php _e( 'CLAS Admin Tools', 'ufclas-admin-tools' ); ?></h2>
		...
		if ( !empty( $_POST[ 'action' ] ) && check_admin_referer( 'ufclas-admin-tools-getusers', '_wpufclasnonce' ) ) {
			...
			if ( isset($_POST['usernames']) ){
				...
				$row_format = '<tr class="%s"><td>%s</td><td>%s</td></tr>';
		...
		?>
            ...
            <form id="ufclas-admin-tools-form" action="" method="post">
            	<p><label for-"usernames"><?php _e( 'Enter list of usernames, one per line.', 'ufclas-admin-tools' ); ?></label></p>
