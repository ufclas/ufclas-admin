...
class Reports_List_Table extends WP_List_Table {
	...
	function column_plugins($item){
       $list = array();
	   foreach ( $item['plugins'] as $plugin_path ){
    ...
    function prepare_items() {
		...
		if ( $order_by == 'status' ) {
			$query .= ' ORDER BY public ';
		} elseif ( $order_by == 'ID' ) {
			$query .= ' ORDER BY blog_id ';
		} elseif ( $order_by == 'path' ) {
			$query .= ' ORDER BY path ';
		} else {
			$order_by = null;
		}
		...
		foreach($results as $active_site){	
			...
			$status = $this->status_names[$active_site['public']];
			$path = $active_site['path'];
			if($id > 1){
				switch_to_blog( $id );
				$data[] = array(
					'ID' => $id, 
